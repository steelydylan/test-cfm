...
describe("ログインテスト",function(){
  it('画像fieldの生成',function(done){
    nightmare.goto("http://horicdesign.com/login")
      .type('[name="mail"]',"hori.godai@gmail.com")
      .type('[name="pass"]',"jdLNXECm")
      .click('[name="ACMS_POST_Login_Auth"]')
      .wait(1000)
      .evaluate(function(){
        return document.querySelector("[name*='ACMS_POST_Logout']").value;
      })
      .then(function(result){
        assert.equal(result,"ログアウト")
        done();
      })
      .catch(function (error) {
        done(error);
